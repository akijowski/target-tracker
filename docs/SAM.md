...
```bash
target-tracker$ sam logs -n StockCheckerFunction --stack-name target-tracker --tail
```
...
```bash
target-tracker$ go test -v ./functions/...
```
...
```bash
aws cloudformation delete-stack --stack-name target-tracker
```
